...
**Q: Boss直聘爬虫怎么登录？boss login 扫码扫不上？**

A: `boss login` 生成的二维码**只能用 Boss直聘 App 扫**，微信/QQ 扫码是不行的！
1. 手机下载「Boss直聘」App
2. 打开 App → 右上角扫一扫
3. 扫终端里显示的二维码，手机确认授权
4. Cookie 会自动保存，后续无需重复登录

如果 boss-cli 不可用（没装/登录失败），系统会自动 fallback 到内置策展数据，不影响基本使用。
...
**Q: 改了城市和关键词还是显示武汉AI的岗位？**

A: 已修复。之前策展数据没有按城市/关键词过滤。现在 `search_boss_jobs(keyword, city)` 会**同时按关键词和城市**过滤。如果你搜「北京 + Java」但策展数据里没有匹配项，会返回空列表（这是正确行为）。真实爬虫（boss-cli/牛客/猎聘）会正常按你的参数搜索。
