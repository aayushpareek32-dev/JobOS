"""Boss直聘 job scraper using curated data + web search."""
...
def get_known_wuhan_ai_jobs() -> list[dict]:
    """Return curated Boss直聘 AI/Agent jobs in Wuhan area."""
    return [
        {
            "platform": "boss",
            "job_id": "boss_hw_001",
        ...
        },
        {
            "platform": "boss",
            "job_id": "boss_ks_001",
            "title": "应用算法实习生（LLM/Agent/RAG方向）",
            "company": "金山软件",
            "location": "武汉/北京",
            "salary": "实习补贴+免费公寓",
            "job_type": "暑期实习",
            "description": "参与大模型应用算法研发，涉及LLM、Agent、RAG等方向的算法设计与工程落地。金山软件集团2026-2028届算法实习生招聘",
            "requirements": "研究生及以上学历(优秀本科生可);全职实习6个月以上;熟悉Python;了解PyTorch/HuggingFace;有大模型相关项目经验优先",
            "url": "https://campus.wps.cn/",
            "posted_date": "2026-03",
        },
        {
            "platform": "boss",
...
def search_boss_jobs(keyword: str = "AI Agent", city: str = "武汉") -> list[dict]:
    """Aggregate curated Boss直聘 job data."""
    all_jobs = get_known_wuhan_ai_jobs()
    ...
    for job in all_jobs:
        ...
        if any(k in text for k in [kw_lower, "agent", "大模型", "llm", "ai"]):
            results.append(job)
